...
void main (void) {
  ...
  /*
    Assemble
  */
  vec3 plane = vec3(0.211, 0.211, 0.0);
  ...
  if (!isInvalid) {
    ...
    float dimension3D = (pow(totalSquares, 1.0 / 3.0));

    float d3X = mod(abs(squareIDX / pow(dimension3D, 0.0)), dimension3D) - dimension3D * 0.5;
    float d3Y = mod(abs(squareIDX / pow(dimension3D, 1.0)), dimension3D) - dimension3D * 0.5;
    float d3Z = mod(abs(squareIDX / pow(dimension3D, 2.0)), dimension3D) - dimension3D * 0.5;
